...
def convert():
    ...
    json_csv_parser.add_argument(
        "--in", dest="input", required=True, help="путь к файлу ввода"
    )
    json_csv_parser.add_argument(
        "--out",
        dest="output",
        required=True,
        help="путь в файлу куда будет записан результат",
    )

    csv_json_parser = sub.add_parser("csv2json")
    csv_json_parser.add_argument(
        "--in", dest="input", required=True, help="путь к файлу ввода"
    )
    csv_json_parser.add_argument(
        "--out",
